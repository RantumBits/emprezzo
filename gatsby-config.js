    ...
    },
    {
      ...
      options: {
        ...
        queries: [
          ...
          {
            statement: 'SELECT  CONCAT(VendorURL, FLOOR(RAND() * 10000)) AS UniqueKey,    CONCAT(SUBSTRING(VendorURL,9,9), ShopifyProductsAll.ProductID) AS UniqueID, ShopifyProductsAll.*,    ShopifyDesc.Description as productDesc,    Tags.*,    PayNShip.*,    SocialSummary.*, AlexaRankView.* FROM    ShopifyProductsAll        LEFT JOIN    ShopifyDesc ON ShopifyDesc.ProductID = ShopifyProductsAll.ProductID        LEFT JOIN    Tags ON TRIM(TRAILING  '/' FROM ShopifyProductsAll.VendorURL) = TRIM(TRAILING  '/' FROM Tags.url)    LEFT JOIN    PayNShip ON TRIM(TRAILING  '/' FROM ShopifyProductsAll.VendorURL) = TRIM(TRAILING  '/' FROM PayNShip.URL)    LEFT JOIN    SocialIDView ON TRIM(TRAILING  '/' FROM ShopifyProductsAll.VendorURL) = TRIM(TRAILING  '/' FROM SocialIDView.URL)	 LEFT JOIN    SocialSummary ON TRIM(TRAILING  '/' FROM SocialIDView.Instagram) = TRIM(TRAILING  '/' FROM SocialSummary.Instagram) LEFT JOIN    AlexaRankView ON TRIM(TRAILING  '/' FROM ShopifyProductsAll.VendorURL) = TRIM(TRAILING  '/' FROM AlexaRankView.URL) GROUP BY UniqueID LIMIT 1000',
            idFieldName: 'UniqueKey',
